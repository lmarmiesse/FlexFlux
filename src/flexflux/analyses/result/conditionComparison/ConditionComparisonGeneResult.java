...
public class ConditionComparisonGeneResult {
	...
	public ConditionComparisonGeneResult(Objective o, Condition c,
			...
			ConditionComparisonFvaResult fvaResult, BioNetwork network) {
		this.objective = o;
		this.condition = c;
		this.koResult = koResult;
		this.fvaResult = fvaResult;

		HashMap<String, BioChemicalReaction> allReactions = network
		...
		if (objValue != 0) {
			...
			System.err.println("Potentially used genes : "
					+ potentiallyUsedGenes);
			...
			for (String geneId : potentiallyUsedGenes.keySet()) {
				if (!essentialGenes.containsKey(geneId)) {
					dispensableGenes.put(geneId,
							potentiallyUsedGenes.get(geneId));
				}
			}
